...
export function AccountCard({ account }: AccountCardProps) {
  ...
  const { balance, spent, budgetTotal, currentPeriod, completion } = useMemo(() => {
    const now = new Date();
      ...
      <CardContent className="relative z-10 space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="relative h-8 bg-muted rounded-full overflow-visible border-2"
              style={{
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className={cn(
              "font-semibold",
              budgetTotal - spent >= 0 ? "text-green-600" : "text-red-600"
            )}>
              {maskValue(new Intl.NumberFormat("pt-BR", {
                style: "currency",
                currency: account.currency,
