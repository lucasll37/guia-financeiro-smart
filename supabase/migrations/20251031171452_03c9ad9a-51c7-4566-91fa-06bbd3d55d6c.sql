-- Função para sincronizar lançamentos de receita de cartão em contas casa
...
BEGIN
  ...
  -- Buscar subcategoria de receita do usuário
...
-- Criar trigger para sincronizar receitas de cartão em contas casa
DROP TRIGGER IF EXISTS tr_sync_casa_cc_revenues_on_insert ON public.transactions;
CREATE TRIGGER tr_sync_casa_cc_revenues_on_insert
AFTER INSERT ON public.transactions
FOR EACH ROW
EXECUTE FUNCTION public.sync_casa_credit_card_revenues();

DROP TRIGGER IF EXISTS tr_sync_casa_cc_revenues_on_update ON public.transactions;
CREATE TRIGGER tr_sync_casa_cc_revenues_on_update
AFTER UPDATE ON public.transactions
FOR EACH ROW
WHEN (
  OLD.credit_card_id IS DISTINCT FROM NEW.credit_card_id OR
  OLD.payment_month IS DISTINCT FROM NEW.payment_month OR
  OLD.amount IS DISTINCT FROM NEW.amount OR
  OLD.account_id IS DISTINCT FROM NEW.account_id
)
EXECUTE FUNCTION public.sync_casa_credit_card_revenues();

DROP TRIGGER IF EXISTS tr_sync_casa_cc_revenues_on_delete ON public.transactions;
CREATE TRIGGER tr_sync_casa_cc_revenues_on_delete
