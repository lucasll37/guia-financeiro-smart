-- Adicionar coluna created_by na tabela credit_cards
ALTER TABLE public.credit_cards 
ADD COLUMN created_by uuid REFERENCES auth.users(id);

-- Backfill: para cartões existentes, definir o owner da conta como criador
UPDATE public.credit_cards cc
SET created_by = a.owner_id
FROM public.accounts a
WHERE cc.account_id = a.id AND cc.created_by IS NULL;
...
CREATE INDEX idx_credit_cards_created_by ON public.credit_cards(created_by);

-- Atualizar a função sync_casa_credit_card_revenues para usar created_by do cartão
